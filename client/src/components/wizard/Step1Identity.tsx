...
export function Step1Identity({ data, updateData, onNext }: Props) {
  ...
  const handleFileUpload = async (field: "idFront" | "idBack" | "passportPhoto", e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleContinue = async () => {
    ...
    if (docType === "id_card" && (!data.idFront || !data.idBack)) {
      setError("გთხოვთ, ატვირთოთ პირადობის მოწმობის ორივე მხარე");
      return;
    } else if (docType === "passport" && !data.passportPhoto) {
      setError("გთხოვთ, ატვირთოთ პასპორტის ფოტო");
      return;
    }
    ...
    try {
      const endpoint = docType === "passport" ? "/api/vision/extract-passport" : "/api/vision/extract-id";
      const payload = docType === "passport" ? { passportImage: data.passportPhoto } : { frontImage: data.idFront, backImage: data.idBack };
  ...
  const canProceed = docType === "id_card" 
    ? !!data.idFront && !!data.idBack && !isScanning
    : docType === "passport"
    ? !!data.passportPhoto && !isScanning
    : false;
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <Button 
          ...
          onClick={() => { setDocType("id_card"); updateData({ documentType: "id_card" }); setScanComplete(false); setError(null); }}
        ...
        <Button 
          ...
          onClick={() => { setDocType("passport"); updateData({ documentType: "passport" }); setScanComplete(false); setError(null); }}
          className="h-16 text-lg rounded-2xl"
        >
          პასპორტი
        </Button>
      ...
      {docType && (
        ...
        <div className="relative group" ref={backRef}>
          ...
          <label className={cn(
            ...
            <div className="relative group sm:col-span-2" ref={passportRef}>
              <Label className={cn("block mb-2 font-semibold", errors.passportPhoto && "text-destructive")}>პასპორტის ფოტო</Label>
              <label className={cn(
                "flex flex-col items-center justify-center w-full h-40 border-2 border-dashed rounded-2xl cursor-pointer transition-all duration-200",
                data.passportPhoto ? "border-primary bg-primary/5" : "border-border hover:border-primary/50 hover:bg-muted/50",
                errors.passportPhoto && "border-destructive bg-destructive/5"
              )}>
                {data.passportPhoto ? (
                  <div className="w-full h-full relative overflow-hidden rounded-2xl">
                    <img
                      src={getPreviewSrc(data.passportPhoto)}
                      alt="Passport"
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/70 to-transparent">
                      <div className="flex items-center gap-2">
                        <CheckCircle2 className="w-5 h-5 text-white" />
                        <p className="text-sm font-medium text-white">პასპორტი აიტვირთა (შეცვლა)</p>
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    <UploadCloud className="w-10 h-10 text-muted-foreground mb-3 group-hover:text-primary transition-colors" />
                    <p className="text-sm font-medium text-foreground">დააჭირეთ პასპორტის ასატვირთად</p>
                  </div>
                )}
                <input type="file" className="hidden" accept="image/*" onChange={(e) => handleFileUpload("passportPhoto", e)} />
              </label>
            </div>
          )}
        </div>
